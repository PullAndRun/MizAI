...
async function follow(uname: string, event: groupMessageEvent) {
  if (!uname) {
  ...
  await biliModel.findOrAdd(user.uname, event.groupId, user.mid, user.room_id);
  await sendGroupMsg(event.groupId, [
    Structs.reply(event.messageId),
    Structs.text(`关注成功\n已为您关注主播 "${uname}"`),
  ]);
...
async function unfollow(uname: string, event: groupMessageEvent) {
  if (!uname) {
    await sendGroupMsg(event.groupId, [
      Structs.reply(event.messageId),
      Structs.text(
  ...
  await biliModel.remove(event.groupId, uname);
  await sendGroupMsg(event.groupId, [
    Structs.reply(event.messageId),
    Structs.text(`取关成功\n已为您取关主播 "${uname}"`),
  ]);
...
async function list(_: string, event: groupMessageEvent) {
...
async function query(uname: string, event: groupMessageEvent) {
  if (!uname) {
    await sendGroupMsg(event.groupId, [
      Structs.reply(event.messageId),
      Structs.text(
